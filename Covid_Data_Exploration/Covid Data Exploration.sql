...
select location, max(convert(int,total_deaths))  from portfolio.coviddeaths group by location order by max(convert(int,total_deaths) desc;

select distinct(continent) from portfolio.coviddeaths;

 --max deaths in each continent                                                                                                          
